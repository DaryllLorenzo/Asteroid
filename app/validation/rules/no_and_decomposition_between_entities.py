...
class NoAndDecompositionBetweenEntities(Rule):
    """
    No And Decomposition Between Entities.
    ...
    def check(self, context: dict) -> str | None:
        ...
        """
        if (
            context.get("arrow_type") == "and_decomposition"
            and context.get("source_is_entity")
            and context.get("dest_is_entity")
        ):
            return tr(
                "Cannot create an AND Decomposition between Actors/Agents."
                " Links are for Tropos elements inside the subcanvas."
            )
        return None


rule = NoAndDecompositionBetweenEntities()
